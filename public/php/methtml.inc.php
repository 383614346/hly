...
//sign unsign
function methtml_getunsign($newid){
	session_start();
	global $db;
	global $_M;
	 
	$newssignner2 = $db->get_one("select * from pol_news where id=".$newid);
	...
	$unstr_sign = trim($str_sign,',');
	
	$un=explode(',',$unstr_sign);
	$_M['unsignnername']="";
	foreach($un as $id){
		$unuser=$db->get_one("select * from pol_admin_table where id=".$id);
		$_M['unsignnername'].=$unuser['admin_id'].'   ';
	}

}
//getnews
function methtml_getnewsarray($usernameid){
	
	global $db;
	$newsarray = $db->get_one("select * from pol_news where getterid like '%".$usernameid."%'   and (recycle='0' or recycle='-1') and signnerid not like '%".$usernameid."%'");
	return $newsarray;
}

//signer
function methtml_getsign($nid){
	session_start();
	global $db;
	global $_M;
	$news['id']=$nid;
	...
	foreach($sigar as $id){
		...
		$_M['signnername'].=$user['admin_id'].'   ';
	}
	
}
//getone
function methtml_getonedata($id,$signnerid){
	global $db;
	global $_M;
	$list['id']=$id;
...
function methtml_getarray($mark,$type,$order,$module,$listmx=-1,$para=0,$categoryname=0,$marktype=0,$txtmax=0,$descmax=0,$rand=0){
	...
	$numname = ' id,title,description,class1,class2,class3,updatetime,addtime,filename,access,top_ok,hits,issue,com_ok,no_order,signnerid,getterid,';
