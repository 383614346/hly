...
//			$list[] = "<input type=\"text\" name=\"no_order-{$val[id]}\" class=\"ui-input met-center\" style=\"width:20px;\" value=\"{$val[no_order]}\">";
			$list[] = $key+1;
			$list[] = "<a href=\"{$val[title]}\">{$val[title]}</a>";
			$list[] = $val[displaytype]?'审核':'未审核';
			$list[] = $val[com_ok]?'是':'否';
			$list[] = $val[top_ok]?'是':'否';
...
//			$list[] = "<a href=\"{$_M[url][own_form]}a=do_editor&id={$val[id]}\" class=\"edit\">编辑</a><span class=\"line\">|</span><a href=\"{$_M[url][own_form]}a=do_del&id={$val[id]}\" class=\"delet\" data-confirm='{$_M[word][js7]}'>删除</a>";
			$list[] = "<a href=\"{$_M[url][own_form]}a=do_del&id={$val[id]}\" class=\"delet\" data-confirm='{$_M[word][js7]}'>删除</a>";
	...
	function getmydata($serachTime,$serachUpMonthTime){
		global $_M;
		$table = load::sys_class('tabledata', 'new');
		...
		while($rs = mysql_fetch_assoc($re)){
			for ($i=0; $i<count($list); $i++) {
				if($list[$i][issue] == $rs[issue]){	
					$list[$i][bycount] = $rs[count];
				}
			} 
		}
