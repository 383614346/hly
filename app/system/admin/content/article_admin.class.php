...
class article_admin extends admin {
	...
	function doadd() {
		...
		$list["adminid"] = get_met_cookie('metinfo_admin_id');
	...
	function doeditor() {
		...
		$articletime1=db::get_one("SELECT * FROM pol_signtime WHERE signnerid LIKE '%1%' and newsid='".$list['id']."'");
		$articletime2=db::get_one("SELECT * FROM pol_signtime WHERE signnerid LIKE '%2%' and newsid='".$list['id']."'");
		$articletime3=db::get_one("SELECT * FROM pol_signtime WHERE signnerid LIKE '%3%' and newsid='".$list['id']."'");
		$articletime4=db::get_one("SELECT * FROM pol_signtime WHERE signnerid LIKE '%4%' and newsid='".$list['id']."'");
		$articletime5=db::get_one("SELECT * FROM pol_signtime WHERE signnerid LIKE '%5%' and newsid='".$list['id']."'");
		$articletime6=db::get_one("SELECT * FROM pol_signtime WHERE signnerid LIKE '%6%' and newsid='".$list['id']."'");
		$articletime7=db::get_one("SELECT * FROM pol_signtime WHERE signnerid LIKE '%7%' and newsid='".$list['id']."'");
		$articletime8=db::get_one("SELECT * FROM pol_signtime WHERE signnerid LIKE '%8%' and newsid='".$list['id']."'");
		if($articletime1['time']==null){
			$articletime1['time']="未签收";
		}
		if($articletime2['time']==null){
			$articletime2['time']="未签收";
		}
		if($articletime3['time']==null){
			$articletime3['time']="未签收";
		}
		if($articletime4['time']==null){
			$articletime4['time']="未签收";
		}
		if($articletime5['time']==null){
			$articletime5['time']="未签收";
		}
		if($articletime6['time']==null){
			$articletime6['time']="未签收";
		}
		if($articletime7['time']==null){
			$articletime7['time']="未签收";
		}
		if($articletime8['time']==null){
			$articletime8['time']="未签收";
		}
		require $this->template('tem/article_add');
	}
	...
	//getone
	function dogetonedata($id,$signnerid){
		global $db;
		global $_M;
		$list['id']=$id;
		return $db->get_one("SELECT * FROM pol_signtime WHERE signnerid='".$signnerid."' and newsid='".$list['id']."'");
	}
	function doindex() {
		global $_M;
